...
<div class="bg-white border-b sticky top-16 z-40 shadow-sm">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <form method="GET" action="{{ route('zenithalms.courses.index') }}" class="space-y-4">
            <!-- Mobile: Stack vertically -->
            <div class="flex flex-col space-y-3 lg:hidden w-full">
                <!-- Search Input -->
                <div class="relative w-full">
                    <input type="text"
                           ...
                           class="w-full pl-10 pr-4 py-2 border border-neutral-300 rounded-xl focus:outline-none focus:border-primary-500 break-words">
                    <span class="material-icons-round absolute left-3 top-2.5 text-neutral-400">search</span>
                </div>

                <!-- Filter Dropdowns -->
                <div class="grid grid-cols-2 gap-2 w-full">
                    <select name="category" class="px-4 py-2 border border-neutral-300 rounded-xl focus:outline-none focus:border-primary-500 text-sm w-full">
                    ...
                    <select name="level" class="px-4 py-2 border border-neutral-300 rounded-xl focus:outline-none focus:border-primary-500 text-sm w-full">
                    ...
                    <select name="price_type" class="px-4 py-2 border border-neutral-300 rounded-xl focus:outline-none focus:border-primary-500 text-sm w-full">
                    ...
                    <select name="sort" class="px-4 py-2 border border-neutral-300 rounded-xl focus:outline-none focus:border-primary-500 text-sm w-full">
            ...
            <div class="hidden lg:flex lg:flex-col lg:space-y-4 w-full">
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    @if($courses->isEmpty())
        <div class="text-center py-12">
            <div class="material-icons-round text-6xl text-neutral-300 mb-4">search_off</div>
            <h3 class="text-xl font-semibold text-neutral-600 mb-2">No courses found</h3>
            <p class="text-neutral-500">Try adjusting your search criteria or filters.</p>
        </div>
    @else
        <!-- Course Grid -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @foreach($courses as $course)
                <div class="bg-white rounded-2xl border border-neutral-200 overflow-hidden hover:shadow-xl transition-all duration-300 group w-full max-w-full">
                    <!-- Course Image -->
                    <div class="relative h-48 bg-gradient-to-br from-primary-100 to-accent-purple/20 overflow-hidden w-full">
                    ...
                    <div class="p-6 w-full">
                        ...
                        <!-- Title -->
                        <h3 class="text-lg font-bold text-neutral-900 mb-2 line-clamp-2 overflow-hidden">
                            {{ $course->title }}
                        </h3>

                        <!-- Description -->
                        <p class="text-neutral-600 text-sm mb-4 line-clamp-3 overflow-hidden">
                            {{ $course->description }}
                        </p>
                        ...
                        <div class="flex items-center justify-between text-sm text-neutral-500 mb-4">
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <span>{{ $course->duration_minutes ? round($course->duration_minutes / 60) : 0 }}h</span>
                            </div>
                        </div>
