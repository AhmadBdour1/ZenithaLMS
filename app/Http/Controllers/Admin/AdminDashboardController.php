    ...
    private function getPaymentsStats()
    {
        ...
        return [
            ...
            'transactions' => [
                ...
                'successful' => DB::table('transactions')->where('status', 'completed')->count(),
                'failed' => DB::table('transactions')->where('status', 'failed')->count(),
                'pending' => DB::table('transactions')->where('status', 'pending')->count(),
                'today' => DB::table('transactions')->whereDate('created_at', $now->toDateString())->count(),
                'this_week' => DB::table('transactions')->whereBetween('created_at', [$now->startOfWeek(), $now->endOfWeek()])->count(),
                'this_month' => DB::table('transactions')->whereMonth('created_at', $now->month)->whereYear('created_at', $now->year)->count(),
                'total_amount' => DB::table('transactions')->where('status', 'completed')->sum('amount'),
                'today_amount' => DB::table('transactions')->where('status', 'completed')->whereDate('created_at', $now->toDateString())->sum('amount'),
                'this_week_amount' => DB::table('transactions')->where('status', 'completed')->whereBetween('created_at', [$now->startOfWeek(), $now->endOfWeek()])->sum('amount'),
                'this_month_amount' => DB::table('transactions')->where('status', 'completed')->whereMonth('created_at', $now->month)->whereYear('created_at', $now->year)->sum('amount'),
            ],
            'by_payment_method' => DB::table('transactions')
